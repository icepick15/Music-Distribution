...
const SubscriptionPage = () => {
  ...
  const [currentPaymentPlan, setCurrentPaymentPlan] = useState(null); // Track which plan is being paid for
  const [currentPaymentRef, setCurrentPaymentRef] = useState(null); // Track current payment reference
  ...
  const paymentSucceededRef = useRef(false); // Use ref to avoid closure issues
  ...
  // Show emergency actions when payment succeeds (as backup if auto-verification fails)
  useEffect(() => {
    ...
    if (paymentInitiated) {
      console.log('💳 Payment successful - auto-verification triggered, emergency actions available as backup');
      setShowEmergencyActions(true);
    } else {
  ...
  const handleUpgrade = async (plan) => {
    // Reset payment success flag for new payment attempt
    paymentSucceededRef.current = false;
    ...
    try {
      ...
      // Store payment details for automatic verification
      setCurrentPaymentPlan(plan);
      setCurrentPaymentRef(paymentData.reference);
      ...
      const handler = window.PaystackPop.setup({
        ...
        onSuccess: async function(transaction) {
          ...
          paymentSucceededRef.current = true; // Mark payment as succeeded
          ...
          console.log('🔄 Auto-verifying payment with reference:', paymentData.reference);
          try {
            const result = await verifyPayment(paymentData.reference, plan);
            if (!result) {
              console.warn('⚠️ Verification returned false, may need manual verification');
            ...
            // If auto-verification fails, show manual verification option
            toast.error('Auto-verification failed. Please click "Verify Payment" below to complete your subscription.', {
              duration: 10000,
              position: 'top-center'
            });
          }
        ...
        onClose: function() {
          // Paystack calls onClose for both cancel and after success close
          // Only treat as cancel if payment did NOT succeed
          console.log('Paystack modal closed. Payment succeeded:', paymentSucceededRef.current);
          
          if (!paymentSucceededRef.current) {
            console.log('Payment popup closed without success - treating as cancelled');
            ...
            console.log('Payment succeeded - modal closed after success');
            // Reset the success flag for next payment
            setTimeout(() => {
              paymentSucceededRef.current = false;
            }, 2000);
          }
  ...
  const verifyPayment = async (reference, plan) => {
    try {
      ...
      setTimeout(() => {
        ...
        setPaymentModalOpened(false);
        setPaymentInitiated(false);
        // Keep paymentSucceeded true a bit longer to prevent onClose from cancelling
      }, 1000);
    }
  };
